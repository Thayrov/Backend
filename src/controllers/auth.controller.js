...
class AuthController {
  ...
  uploadDocuments = async (req, res, next) => {
    uploadMiddleware(req, res, async err => {
      ...
      try {
        const updatedUser = await this.authService.addDocumentsToUser(userId, documents);
        return res.status(200).json({message: 'Documents uploaded', updatedUser});
      } catch (error) {
  ...
  renderAdminUsers = async (req, res, next) => {
    try {
      ...
      logger.error('Error in renderAdminUsers: ', err);
      return next(
        CustomError.createError({
